...
void kernel(const char* command) {
    ...
    */
    ...
    // Note: PTE_U Means that the page is accessible by user processes
...
/* ---------------- My Helper Functions: ---------------- */
int8_t __pagetable_owner__ = -1; // Global variable to identify the owner of the pagetable in pagetable_alloc()
...
void process_setup(pid_t pid, int program_number) {
    ...
    */
    ...
    //     vamapping mapping = virtual_memory_lookup(processes[pid].p_pagetable, va);
    //     if (mapping.perm != 0) { 
    //         if (virtual_memory_map(processes[pid].p_pagetable, va, mapping.pa, PAGESIZE, 0, NULL) != 0) {
    ...
    for (uintptr_t va = 0; va < PROC_START_ADDR; va += PAGESIZE) {
        ...
        if (mapping.perm != 0) { 
            if (virtual_memory_map(new_pagetable, va, mapping.pa, PAGESIZE, mapping.perm, pagetable_alloc) != 0) {
                ...
                // We have successfully copied the mapping from the old pagetable to the new pagetable
                // Turns out that we don't need to change refcount
...
// pagetable_alloc()
//   Allocate a pagetable with 512 entries.
//   Returns the address of the allocated pagetable, if having enough space.
        ...
        if(processes[i].p_state == P_FREE) {
            ...
            for (uintptr_t va = PROC_START_ADDR; va < MEMSIZE_VIRTUAL; va += PAGESIZE) {
                ...
                if ((mapping.perm & PTE_W) != 0) {
                    ...
                    /*
                    Step 6:
                    Share read-only (code) pages between parent and child processes.
                    */
                    // if (pageinfo[mapping.pn].refcount > 0 && pageinfo[mapping.pn].owner > 0) {
        		    //     ++pageinfo[mapping.pn].refcount;
                    //     virtual_memory_map(processes[i].p_pagetable, va, mapping.pa, PAGESIZE, PTE_P | PTE_W | PTE_U, NULL);
                    // } 
                }
            }
