...
static int program_load_segment(proc* p, const elf_program* ph,
    ...
    /*
    Step 6: Detecting and handling read-only program segments. 
    */
    // If the segment is read-only:
    // if((ph->p_flags & ELF_PFLAG_WRITE) == 0) {
    //     // Iterate over the pages in the segment, and set them to be read-only.
    //     for (uintptr_t addr = va; addr < end_mem; addr += PAGESIZE) {
    //         vamapping mapping = virtual_memory_lookup(p->p_pagetable, addr);
    //         if(mapping.perm & PTE_W) {
    //             // If the page is writable, change it to read-only.
    //             log_printf("Process %d is changing page %p to read-only\n", p->p_pid, addr);
